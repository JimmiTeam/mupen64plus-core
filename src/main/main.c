...
void new_vi(void)
{
    ...
    if (netplay_is_resimulating())
    {
        ...
        // Track re-simulation progress and exit resim mode when caught up
        netplay_resim_advance();
        ...
        if (!netplay_is_resimulating())
        {
            ...
            {
                // Save state so the ring buffer stays populated for future rollbacks.
                rollback_save(&g_dev, frame_manager_get_frame_index());
            }
        }
        return;
    }
